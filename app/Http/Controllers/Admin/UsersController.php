...
class UsersController extends Controller
{
    ...
    public function editStudents(User $user)
    {
        abort_if(Gate::denies('user_edit'), Response::HTTP_FORBIDDEN, '403 Forbidden');
        ...
        return view('admin.users.students.edit', compact('roles', 'classes', 'user'));
    }
    ...
    public function updateStudents(UpdateUserRequest $request, User $user)
    {
        $user->update($request->all());
        $user->roles()->sync($request->input('roles', []));
    ...
    public function getAllStudents() {
        $studens = User::whereHas('roles', function ($query) {
            $query->where('id', 4);
        })->get();
        ...
        // dd($studens);
        return view('admin.users.students.index', compact('studens'));
    }
